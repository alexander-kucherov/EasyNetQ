...
public static class ConcurrentDictionaryExtensions
{
    ...
    ///     doing so can result in application failures when updating to a new EasyNetQ release.
    /// </summary>
    public static async ValueTask ClearAndDisposeAsync<TKey, TValue>(this ConcurrentDictionary<TKey, TValue> source)
        where TValue : IAsyncDisposable where TKey : notnull
    {
        await source.ClearAndDisposeAsync(async x => await x.DisposeAsync());
    }
    ...
    ///     doing so can result in application failures when updating to a new EasyNetQ release.
    /// </summary>
    public static async ValueTask ClearAndDisposeAsync<TKey, TValue>(
        this ConcurrentDictionary<TKey, TValue> source, Func<TValue, ValueTask> disposeAsync) where TKey : notnull
    {
        do
        {
            foreach (var key in source.Select(x => x.Key))
